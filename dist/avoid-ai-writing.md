...
**Invocation.** Natural language is enough ("rewrite this in a blunt voice for LinkedIn," "edit `post.md` in place," "scan this, don't rewrite"). Power users can also pass explicit options, which map to the sections below: `[--mode rewrite|detect|edit]`, `[--voice casual|professional|technical|warm|blunt]`, `[--context linkedin|blog|technical-blog|investor-email|docs|casual]` (when calling the deterministic detector or gate, map each profile to `contextMode` per **Skill profiles and detector contextMode** in Context profiles below), `[--file PATH]`, `[--iterate N]` (max 2), `[--style CONFIG|GUIDE]`.
